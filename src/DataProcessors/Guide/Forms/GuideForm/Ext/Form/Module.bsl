...
Function GenerateGuideHTML(Section)
	...
	If Not Section = "" Then
		...
		SectionDocument = DataProcessors.Guide.GetTemplate(Section).GetHTMLDocument();
...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	For Cnt = 1 To Metadata.DataProcessors.Guide.Templates.Count() - 1 Do

		MetData = Metadata.DataProcessors.Guide.Templates.Get(Cnt);
...
&AtClient
Procedure ShowPartition(Document, Section)
	...
	If Not TemporaryAnchor = Undefined Then
...
Procedure HTMLFieldOnClick(Element, EventData, StandardProcessing)
	...
	If Not EventData.Anchor = Undefined Then
		...
		ElsIf EventData.Anchor.id = "LeftArrow" Then
			...
			ShowPartition(CurrentDocument, ConfigurationSections[Index].Title);
		...
		ElsIf EventData.Anchor.id = "RightArrow" Then
			...
			ShowPartition(CurrentDocument,
			ConfigurationSections.Get(Index).Title);

		Else;
			
			ShowPartition(CurrentDocument, EventData.Anchor.id);
...
Procedure HTMLDocumentGeneratedField(Element)
	...
	ShowPartition(Element.Document, ConfigurationSections[0].Title);
EndProcedure
