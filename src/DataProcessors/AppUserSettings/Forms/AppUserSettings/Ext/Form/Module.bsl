...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	CurrentInfobaseUser = DataProcessors.AppUserSettings.InfobaseUserName(CurrentUserRef);
	...
	If Parameters.User <> Undefined Then
		...
		InfobaseUser = DataProcessors.AppUserSettings.InfobaseUserName(UserRef);
	Else
...
Procedure CopyAllSettings(Command)
	...
	OpenForm("DataProcessor.AppUserSettings.Form.UsersSelection", FormParameters);
...
Procedure CopyFrom(Command)
	...
	OpenForm("DataProcessor.AppUserSettings.Form.CopyAppUserSettings", FormParameters);
...
Procedure FillSettingsLists()
	...
	DataProcessors.AppUserSettings.FillSettingsLists(ThisObject);
...
Procedure CopyAtServer(UsersTarget, ReportPersonalizationCount, Report)
	...
	If SelectedSettingsPage = "ReportSettingsPage" Then
		...
		DataProcessors.AppUserSettings.CopyReportAndPersonalSettings(ReportsUserSettingsStorage,
			InfobaseUser, UsersTarget, Result.SettingsArray, NotCopiedReportSettings);
		// Copying report options
		DataProcessors.AppUserSettings.CopyReportOptions(Result.ReportOptionArray,
			UserReportOptionTable, InfobaseUser, UsersTarget);
			
		If NotCopiedReportSettings.Count() <> 0
			OR UserReportOptionTable.Count() <> 0 Then
			Report = DataProcessors.AppUserSettings.CreateReportOnCopyingSettings(
				NotCopiedReportSettings, SelectedReportOptionsTable);
		EndIf;
		
	ElsIf SelectedSettingsPage = "AppearancePage" Then
		DataProcessors.AppUserSettings.CopyAppearanceSettings(InfobaseUser, UsersTarget, Result.SettingsArray);
	Else
		
		If Result.PersonalSettingsArray.Count() <> 0 Then
			DataProcessors.AppUserSettings.CopyReportAndPersonalSettings(CommonSettingsStorage,
			...
			For Each OtherUserSettings In Result.UserSettingsArray Do
				For Each UserTarget In UsersTarget Do
					...
					UserInfo.Insert("InfobaseUserName",
						DataProcessors.AppUserSettings.InfobaseUserName(UserTarget));
		...
		DataProcessors.AppUserSettings.CopyAppearanceSettings(
...
Procedure CopyAllSettingsOnServer(User, UsersTarget, SettingsArray, Report)
	...
	NotCopiedReportSettings.Columns.Add("User");
	NotCopiedReportSettings.Columns.Add("ReportList", New TypeDescription("ValueList"));
	DataProcessors.AppUserSettings.CopyAppUserSettings(
...
Procedure ClearOnServer(Users = Undefined, ToSelectedUsers = False)
	...
	If ToSelectedUsers Then
		...
		DataProcessors.AppUserSettings.DeleteSettingsForSelectedUsers(Users, Result.SettingsArray, StorageName);
		If Result.PersonalSettingsArray.Count() <> 0 Then
			DataProcessors.AppUserSettings.DeleteSettingsForSelectedUsers(Users,
	...
	// Clearing settings
	DataProcessors.AppUserSettings.DeleteSelectedSettings(InfobaseUser, Result.SettingsArray, StorageName);
	If Result.PersonalSettingsArray.Count() <> 0 Then
		DataProcessors.AppUserSettings.DeleteSelectedSettings(
	...
	If SelectedSettingsPage = "ReportSettingsPage" Then
		...
		DataProcessors.AppUserSettings.DeleteReportOptions(
...
Procedure ClearAllSettingsOnServer(SettingsToClear)
	...
	DataProcessors.AppUserSettings.DeleteAppUserSettings(
...
Procedure ClearAllUserSettingsOnServer()
	...
	UserTable = DataProcessors.AppUserSettings.UsersToCopy("", UserTable, UseExternalUsers);
	
	For Each TableRow In UserTable Do
		UserArray.Add(TableRow.User);
	EndDo;
	
	DataProcessors.AppUserSettings.DeleteAppUserSettings(SettingsToClear, UserArray, UserReportOptionTable);
