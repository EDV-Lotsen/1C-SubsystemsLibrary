...
Procedure FillUserList(UserType, UseGroups);
	...
	If UseGroups Then
		DataProcessors.AppUserSettings.FillGroupTree(GroupTree, ExternalUser);
		AllUsersListTable = DataProcessors.AppUserSettings.UsersToCopy(
			UserSource, AllUsersListTable, ExternalUser);
	Else
		UserListTable = DataProcessors.AppUserSettings.UsersToCopy(
