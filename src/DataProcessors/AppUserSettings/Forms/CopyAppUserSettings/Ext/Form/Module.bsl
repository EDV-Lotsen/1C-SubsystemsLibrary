...
Procedure SelectSettings(Item)
	...
	OpenForm("DataProcessor.AppUserSettings.Form.SelectSettings", FormParameters);
...
Procedure Copy(Command)
	...
	If CheckResult = "HasActiveUsersRecipients" Then
		...
		If SettingsToCopyRadioButton = "CopyAll" 
			...
			OpenForm("DataProcessor.AppUserSettings.Form.SettingsCopyingWarning", FormParameters);
			Return;
...
Procedure CopySelectedSettings(Report)
	...
	User = DataProcessors.AppUserSettings.InfobaseUserName(UserRef);
	...
	ElsIf CopySettingsToRadioButton = "AllUsers" Then
		...
		DataProcessors.AppUserSettings.UsersToCopy(UserRef, UserTable,
	...
	If SelectedSettings.ReportSettings.Count() > 0 Then
		...
		DataProcessors.AppUserSettings.CopyReportOptions(
	...
	If SelectedSettings.Appearance.Count() > 0 Then
		DataProcessors.AppUserSettings.CopyAppearanceSettings(User, Targets, SelectedSettings.Appearance);
	EndIf;
	
	If SelectedSettings.OtherSettings.Count() > 0 Then
		DataProcessors.AppUserSettings.CopyAppearanceSettings(User, Targets, SelectedSettings.OtherSettings);
	EndIf;
	...
	If SelectedSettings.OtherUserSettings.Count() > 0 Then
		...
		For Each CatalogUser In Targets Do
			...
			UserInfo.Insert("InfobaseUserName", 
				DataProcessors.AppUserSettings.InfobaseUserName(CatalogUser));
...
Function CopyingAllSettings()
	...
	Else
		...
		UserTable = New ValueTable;
		UserTable.Columns.Add("User");
		UserTable = DataProcessors.AppUserSettings.UsersToCopy(UserRef, UserTable, 
			TypeOf(UserRef) = Type("CatalogRef.ExternalUsers"));
	...
	SettingsCopied  = DataProcessors.AppUserSettings.
