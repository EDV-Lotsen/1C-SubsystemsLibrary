...
Procedure FillTableOfOperationsWithObject() Export
	...
	// Try to know, if object has attributes to lock (subsystem of object attributes
	// edit prohibition). Do do this we try to call object method GetObjectLockableAttributes
	// in lock Try .. Except
	Try
		LockableAttributesDescription = ObjectManager.GetObjectLockableAttributes();
		For Each LockableAttributeDescription In LockableAttributesDescription Do
			LockableAttributes.Add( TrimAll (
				StringFunctionsClientServer.DecomposeStringIntoSubstringsArray(
					LockableAttributeDescription, ";")[0] ) );
		EndDo;
	Except
	EndTry;
	
	Try
		NotEditable = ObjectManager.GetNotEditableInteractivelyAttributes();
	Except
