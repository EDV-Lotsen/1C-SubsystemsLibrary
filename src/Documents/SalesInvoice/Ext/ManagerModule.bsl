...
Function Query_OrdersRegistered(TablesList)
	...
	|		ON  OrdersRegisteredBalance.Counterparty = LineItems.Ref.Counterparty
	|		AND OrdersRegisteredBalance.Order   = LineItems.Order
	|		AND OrdersRegisteredBalance.Item = LineItems.Item
	|WHERE
	|	LineItems.Ref = &Ref
	|   AND LineItems.Order <> VALUE(Document.SalesOrder.EmptyRef)
	|ORDER BY
...
Function Query_OrdersRegistered_Balance(TablesList)
	...
	// ------------------------------------------------------
	// Dimensions
	|	OrdersRegisteredBalance.Counterparty     AS Counterparty,
...
Procedure CheckCloseParentOrders(DocumentRef, AdditionalProperties, TempTablesManager)
	...
	_DemoDocumentPosting.PutTemporaryTable(AdditionalProperties.Posting.PostingTables.Table_OrdersRegistered, "Table_OrdersRegistered", Query.TempTablesManager);
	...
	// ------------------------------------------------------
	// Dimensions
	|	OrdersRegistered.Counterparty,
	|	OrdersRegistered.Order,
	|	OrdersRegistered.Item,
	// ------------------------------------------------------
	// Resources
	...
	|	OrdersRegisteredBalance.Order            AS Order,
	|	OrdersRegisteredBalance.Item          AS Item,
	|	OrdersRegisteredBalance.Item.Type     AS Type,
	// ------------------------------------------------------
	// Resources
	...
	|	OrdersRegisteredBalance.Item.Type";
	QueryText   = QueryText + _DemoDocumentPosting.GetDelimiterOfBatchQuery();
	QueryTables = QueryTables + 1;
...
Function Query_Filling_Document_SalesOrder_Attributes(TablesList)
	...
	QueryText =
		...
		|	SalesOrder.CounterpartyCode             AS CounterpartyCode,
		...
		|	CASE
		|		WHEN SalesOrder.Counterparty.Terms.Days IS NULL THEN DATEADD(&Date, DAY, 14)
		|		WHEN SalesOrder.Counterparty.Terms.Days = 0     THEN DATEADD(&Date, DAY, 14)
		|		ELSE                                            DATEADD(&Date, DAY, SalesOrder.Counterparty.Terms.Days)
		|	END                                     AS DueDate,
		|	ISNULL(SalesOrder.Counterparty.Terms, VALUE(Catalog.PaymentTerms.EmptyRef))
		|	                                        AS Terms,
...
Function Query_Filling_Document_SalesOrder_OrdersRegistered(TablesList)
	...
	QueryText =
		...
		// ------------------------------------------------------
		// Dimensions
		|	OrdersRegisteredBalance.Counterpart      AS Counterparty,
		|	OrdersRegisteredBalance.Order            AS Order,
		|	OrdersRegisteredBalance.Item          AS Item,
		// ------------------------------------------------------
		// Resources                                                                                                        // ---------------------------------------
		...
		|						ELSE 0 END                                                                                  //     |
		|				WHEN OrdersRegisteredBalance.Item.Type = VALUE(Enum.InventoryTypes.NonInventory) THEN            //   Non-inventory:
		...
		|			(SELECT
		|				SalesOrderLineItems.Ref.Counterparty,
...
Function Query_Filling_Document_SalesOrder_LineItems(TablesList)
	...
	QueryText =
		...
		|	SalesOrderLineItems.Item             AS Item,
		|	SalesOrderLineItems.ItemDescription  AS ItemDescription,
		...
		|	SalesOrderLineItems.Ref.Counterparty    AS Counterparty
		|INTO
		|	Table_Document_SalesOrder_LineItems
		|FROM
		|	Document.SalesOrder.LineItems AS SalesOrderLineItems
		|	LEFT JOIN Table_Document_SalesOrder_OrdersRegistered AS OrdersRegistered
		|		ON  OrdersRegistered.Counterparty = SalesOrderLineItems.Ref.Counterparty
		|		AND OrdersRegistered.Order   = SalesOrderLineItems.Ref
		|		AND OrdersRegistered.Item = SalesOrderLineItems.Item
		|	LEFT JOIN Table_Document_SalesOrder_OrdersStatuses AS OrdersStatuses
...
Function Query_Filling_Document_SalesOrder_Totals(TablesList)
	...
	QueryText =
		...
		|
		|	CAST( // Format(Total(TaxableAmount) * Counterparty.SalesTaxCode.TaxRate / 100, ""ND=15; NFD=2"")
		|		SUM(SalesOrderLineItems.TaxableAmount) *
		|		ISNULL(SalesOrderLineItems.Counterparty.SalesTaxCode.TaxRate, 0) /
		|		100
		...
		|	SalesOrderLineItems.Counterparty.SalesTaxCode.TaxRate,
		|	SalesOrder.ExchangeRate,
...
Function Query_Filling_Attributes(TablesList)
	...
	If TablesList.Property("Table_Document_SalesOrder_Attributes") Then
		...
		|	Document_SalesOrder_Attributes.Counterparty,
		|	Document_SalesOrder_Attributes.CounterpartyCode,
...
Function Query_Filling_LineItems(TablesList)
	...
	If TablesList.Property("Table_Document_SalesOrder_LineItems") Then
		...
		|	Document_SalesOrder_LineItems.Item,
		|	Document_SalesOrder_LineItems.ItemDescription,
...
Function CheckStatusOfSalesOrder(Ref) Export
	...
	If Not StatusOK Then
		MessageText = NStr("en = 'Failed to generate the invoice based on %1 %2.'");
...
Procedure Print(ObjectArray, PrintParameters, PrintFormsCollection,
    ...
    If _DemoPrintManagement.NeedToPrintTemplate(PrintFormsCollection, "SalesInvoice") Then
...
Function PrintTemplate(ObjectArray, PrintObjects)
   ...
   Selection = Query.Execute().Choose();
	...
	Template = _DemoPrintManagement.GetTemplate("Document.SalesInvoice.PF_MXL_SalesInvoice");
