...
//
Procedure FillDocumentList(Counterparty)
	...
	Query.Text = "SELECT
				 ...
				 |			Account IN (SELECT ARAccount FROM Document.SalesInvoice UNION SELECT APAccount FROM Document.PurchaseReturn WHERE Counterparty = &Counterparty),
				 |			,
				 |			ExtDimension1 = &Counterparty
...
//
Procedure FillCreditMemos(Counterparty)
	...
	Query.Text = "SELECT
				 ...
				 |  // Get due rests from accounting balance
				 |	AccountingRegister.GeneralJournal.Balance(&Date,
				 |			Account IN (SELECT ARAccount FROM Document.SalesReturn WHERE Counterparty = &Counterparty AND ReturnType = VALUE(Enum.ReturnTypes.CreditMemo)),
				 |			,
				 |			ExtDimension1 = &Counterparty AND ExtDimension2 REFS Document.SalesReturn AND ExtDimension2.ReturnType = VALUE(Enum.ReturnTypes.CreditMemo)) AS GeneralJournalBalance
				 |
				 |	// Calculate exchange rate for a document on a present moment
				 |	LEFT JOIN
				 |		InformationRegister.ExchangeRates.SliceLast(&Date, ) AS ExchangeRates
...
Procedure DistributeCreditMemos(RecalculateCreditMemoPayments = True)
	...
	DefaultCurrency = _DemoGeneralFunctionsCached.DefaultCurrency();
	LineItem = Undefined;
	CurrentInvoice = 0;
	CurrentBalanceRC = 0;
	CurrentBalance = 0;
...
Procedure DistributeCashPayment()
	...
	Amount    = MxPayment;  
	Rate	  = ?(Object.Currency = DefaultCurrency, 1,                  _DemoGeneralFunctions.GetExchangeRate(Object.Date, Object.Currency));
...
&AtClient
// CounterpartyOnChange UI event handler. The procedure repopulates line items
// upon a counterparty change.
//
Procedure CounterpartyOnChange(Item)
	...
	FillDocumentList(Object.Counterparty);
	// Fill in credit memos
    FillCreditMemos(Object.Counterparty);
...
Procedure BeforeWrite(Cancel, WriteParameters)
	...
	If Object.LineItems.Count() = 0 Then
		Message("Cash receipt cannot have empty lines. The system automatically shows unpaid documents of the selected counterparty in the line items");
		Cancel = True;
		Return;
	EndIf;
...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If _DemoGeneralFunctionsCached.FunctionalOptionValue("MultiCurrency") Then	
