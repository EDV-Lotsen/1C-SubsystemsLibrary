...
Procedure ImportIP(File)
	...
	For n= 1 To   TotalNRows Do
		...
		NewIP.Counterparty = Catalogs.Counterparties.FindByDescription(ExcelApp.Sheets(1).Cells(NRows,3).Value);
		NewIP.CounterpartyCode = NewIP.Counterparty.Code;
		...
		NewIP.Currency = _DemoGeneralFunctionsCached.DefaultCurrency();
		...
		NewLine.Currency = _DemoGeneralFunctionsCached.DefaultCurrency();
