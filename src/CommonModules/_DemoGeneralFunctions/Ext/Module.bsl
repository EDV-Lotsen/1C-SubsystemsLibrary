...
// Number - item's price.
//
Function RetailPrice(ActualDate, Item) Export
		
	SelectParameters = New Structure;
	SelectParameters.Insert("Item", Item);
...
Function GetOurCompany() Export
	...
	Return Catalogs.Counterparties.OurCompany;
...
//
Function InventoryAcct(ItemType) Export
	
	If ItemType = Enums.InventoryTypes.Inventory Then
...
Function SearchCounterpartyByCode(CounterpartyCode) Export
	...
	Query = New Query("SELECT
	                  ...
	                  |WHERE
	                  |	Counterparties.Code = &CounterpartyCode");
...
Function GetShipToAddress(Counterparty) Export
...
Function ItemLastCost(Item) Export
...
// Check documents table parts to ensure, that items are unique
			...
			If FilledPropertyValues(DoubledItems, CompareItems) Then
				...
				If DoublesCount <= 10 Then // 10 messages enough to demonstrate that check failed
					...
					For Each Double In DoubledItems Do
						...
						ElsIf TypeOf(Value) = Type("CatalogRef.Counterparties") Then
