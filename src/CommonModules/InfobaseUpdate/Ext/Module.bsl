...
Function UpdateDetailsDocument(Val UpdateHandlers) Export
	...
	UpdatesDetailsTemplate = Metadata.CommonTemplates.Find("UpdateDetails");
	If UpdatesDetailsTemplate <> Undefined Then
		UpdatesDetailsTemplate = GetCommonTemplate(UpdatesDetailsTemplate);
	Else	
		UpdatesDetailsTemplate = New SpreadsheetDocument();
	EndIf;
	...
	For Each Version In UpdateHandlers.Rows Do
		...
		DisplayDetailsChanges(Version.Version, UpdateDetailsDocument, UpdatesDetailsTemplate);
...
// Output description of changes in the current version
//
// Parameters
//  VersionNo  – String - version number, for which we display description from the template
//                          of spreadsheet document UpdatesDetailsTemplate into spreadsheet document
//                          UpdateDetailsDocument
//
Procedure DisplayDetailsChanges(Val VersionNo, UpdateDetailsDocument, UpdatesDetailsTemplate)
	
	Number = StrReplace(VersionNo, ".", "_");
	
	If UpdatesDetailsTemplate.Areas.Find("Header" + Number) = Undefined Then
		Return;
	EndIf;
	
	UpdateDetailsDocument.Put(UpdatesDetailsTemplate.GetArea("Header" + Number));
	UpdateDetailsDocument.StartRowGroup("Version" + Number);
	UpdateDetailsDocument.Put(UpdatesDetailsTemplate.GetArea("Version" + Number));
	UpdateDetailsDocument.EndRowGroup();
	UpdateDetailsDocument.Put(UpdatesDetailsTemplate.GetArea("Indent"));
