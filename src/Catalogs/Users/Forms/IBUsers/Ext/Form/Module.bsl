...
Procedure FillIBUsers(OnFormCreate = False)
	...
	While Selection.Next() Do
		...
		NewRow.ThisIsUser          = Selection.ThisIsUser;
		NewRow.ThisIsExternalUser  = Selection.ThisIsExternalUser;
		...
		NewRow.Picture = GetPictureNoByStatus(NewRow.ProblemCode, NewRow.DeletionMark, Selection.ThisIsExternalUser);
	...
	If Filter = "Users" Then
		RowsToBeDeleted.Add(UsersOfInfBase.FindRows(New Structure("ThisIsUser", False)));
		
	ElsIf Filter = "ExternalUsers" Then
		RowsToBeDeleted.Add(UsersOfInfBase.FindRows(New Structure("ThisIsExternalUser", False)));
...
Procedure OpenUserByRef()
	...
	If CurrentData.ProblemCode = 0 Then
		DoMessageBox(NStr(	"en = 'Specified user is not in linked with a catalog user.
                             |To create Information base user linked to the user or external user check the box ""Allow access to information base"" in the corresponding item form.'"));
		...
		EndIf;
	Else
		OpenForm(?(CurrentData.ThisIsExternalUser,
